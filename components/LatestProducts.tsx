...
function LatestProductCard({ product }: { product: any }) {
  ...
  return (
    <Link href={`/shop/${product.id}`} className="no-underline block relative">
      {/* --- TOAST NOTIFICATION --- */}
      {toast && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[60] w-max animate-in fade-in slide-in-from-top-1 duration-300">
      ...
      >
        <div className={`relative w-full h-[250px] flex items-center justify-center overflow-hidden rounded-sm mb-3 transition-colors duration-300 ${
          isActive ? "bg-[#ebedf3] dark:bg-slate-700" : "bg-[#F7F7F7] dark:bg-slate-800 group-hover:bg-[#ebedf3] dark:group-hover:bg-slate-700"
        }`}>
          ...
          <div className={`absolute bottom-4 left-3 flex flex-col gap-2 transition-all duration-300 z-20 ${
            ...
            <button 
              ...
              disabled={isPending}
              className="p-1.5 bg-white dark:bg-slate-600 text-[#151875] dark:text-white rounded-full hover:bg-[#FB2E86] hover:text-white transition-colors shadow-sm flex items-center justify-center"
            >
            ...
            <button 
              ...
              className={`p-1.5 transition-colors flex items-center justify-center
                ${isFavorited ? "text-[#FB2E86]" : "text-[#151875] dark:text-white hover:text-[#FB2E86]"}`}
            >
