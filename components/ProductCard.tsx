  ...
  return (
    <Link href={`/shop/${product.id}`} className="no-underline">
      <div className={`group transition-all duration-300 ${isList
        ...
        {/* Image Container */}
        <div className={`relative bg-[#F6F7FB] dark:bg-slate-800 flex items-center justify-center overflow-hidden transition-all duration-300  ${isList ? "w-28 h-28 sm:w-32 sm:h-32 shrink-0" : "w-full aspect-square"
          }`}>
          <Image
...
function IconButton({ icon }: { icon: React.ReactNode }) {
  ...
  return (
    <button type="button" className="p-1 bg-white dark:bg-slate-800 rounded-full shadow-sm text-[#151875] dark:text-white hover:bg-[#FB2E86] hover:text-white transition-all border border-gray-100 dark:border-slate-700" onClick={(e) => {
      handleClick(e);
    }}>
      {icon}
    </button>
